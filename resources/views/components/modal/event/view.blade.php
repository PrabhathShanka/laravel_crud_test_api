<div x-data="{ open: false }" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" x-on:open-modal.window="if ($event.detail === 'view-event') open = true" x-on:close-modal.window="if ($event.detail === 'view-event') open = false" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-lg shadow-xl">
            <div class="flex items-start justify-between mb-4">
                <h2 class="text-xl font-semibold text-gray-800" id="view_title"></h2>
                <button @click.prevent="$dispatch('close-modal', 'view-event')" class="text-gray-500 hover:text-gray-700">
